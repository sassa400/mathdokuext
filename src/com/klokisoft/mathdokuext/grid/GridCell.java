package com.klokisoft.mathdokuext.grid;
...
import com.klokisoft.mathdokuext.painter.CagePainter;
import com.klokisoft.mathdokuext.painter.CellPainter;
import com.klokisoft.mathdokuext.painter.MaybeValuePainter;
import com.klokisoft.mathdokuext.painter.Painter;
import com.klokisoft.mathdokuext.painter.UserValuePainter;
import com.klokisoft.mathdokuext.statistics.GridStatistics;
...
public class GridCell {
	...
	private static final String TAG = "MathDokuExt.GridCell";
	...
	public void draw(Canvas canvas, float gridBorderWidth,
			GridInputMode inputMode, int digitValue) {
		// Get cell size
		...
		if (mSelected && mGrid != null && mGrid.isActive()) {
			...
			background = mCellPainter.getOnlyOneBackgroundPaint();
	}
		...
		if ((this.isUserValueSet() && digitValue == 0)
				|| (inputMode == GridInputMode.NORMAL && digitValue != 0)) {
			...
			String userValue = Integer
					.toString(inputMode == GridInputMode.NORMAL
							&& digitValue != 0 ? digitValue : mUserValue);
		...
		if ((mPossibles.size() > 0 && !(inputMode == GridInputMode.NORMAL && digitValue != 0))
				|| (inputMode == GridInputMode.MAYBE && digitValue != 0)) {
			// Temporary alter the possible values in case a swipe digit is
			// selected.
			if (digitValue != 0) {
				if (hasPossible(digitValue)) {
					removePossible(digitValue);
				} else {
					addPossible(digitValue);
				}
			}
			...
			if (mGrid.hasPrefShowMaybesAs3x3Grid()) {
				...
				for (int i = 0; i < mPossibles.size(); i++) {
					...
					for (GridCell cell : mGrid.mCells) 
					{
						...
						if (cell.getUserValue() == possible && 
							 ((cell.getColumn() == mColumn && cell.getRow() != mRow)
								|| (cell.getColumn() != mColumn && cell.getRow() == mRow)))
						{
